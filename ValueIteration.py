...
class CartPole(object):
    ...
    def move(self, action, boxed=False): # binary L/R action
        force = 0.0
        if action > 0:
            force = self.force_mag + np.random.uniform(-1,1)
        else:
            force = -self.force_mag + np.random.uniform(-1,1)
...
if __name__ == '__main__':
    ...
    avg_reward_alt = open("avg_reward_alt.txt", "w")
    avg_reward_rand = open("avg_reward_rand.txt", "w")
    if False:
        for i in range(100):
    ...
    if True:
        ...
        for i in t:
            k = k+1
            print i[2],i[1]
            avg_reward_rand.write(str(i[2]))
            avg_reward_rand.write("\n")
        print k
